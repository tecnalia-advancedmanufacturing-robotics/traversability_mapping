...
private:
  ...
  elevation_msgs::OccupancyElevation occupancyMap2DHeight;  // customized message that includes occupancy map and
                                                            // elevation info
...
public:
  TraversabilityMapping() : nh("~"), pubCount(1), mapArrayCount(0)
  {
    // subscribe to traversability filter
    subFilteredGroundCloud =
        nh.subscribe<sensor_msgs::PointCloud2>("/filtered_pointcloud", 5, &TraversabilityMapping::cloudHandler, this);
  ...
  void updateCellElevation(mapCell_t *thisCell, PointType *point)
  {
    ...
    // Reset cell elevation if variation surpasses a threshold and is lower than previous value
    if (abs(thisCell->elevation - point->z) > abs(thisCell->elevation * 0.5) && thisCell->elevation > point->z)
    {
      //   ROS_INFO("Point elevation reseted.");
      thisCell->elevation = point->z;
      thisCell->elevationVar = pointDistance(robotPoint, *point);
      thisCell->observeTimes = 1;
      return;
    }
    ...
    float R =
        pointDistance(robotPoint, *point) * R_factor;  // measurement noise: R, scale it with dist and observed times
  ...
  bool findPointGridInMap(grid_t *gridOut, PointType *point)
  {
    ...
    if (thisGrid.cubeX >= 0 && thisGrid.cubeX < mapArrayLength && thisGrid.cubeY >= 0 &&
        thisGrid.cubeY < mapArrayLength)
    {
    ...
    else
    {
      ...
      ROS_ERROR("Point cloud is out of elevation map boundary. Change params ->mapArrayLength<-. The program will "
                "crash!");
      return false;
    }
    // sub-map id
    ...
    if (thisGrid.gridX < 0 || thisGrid.gridY < 0 || thisGrid.gridX >= mapCubeArrayLength ||
  ...
  void traversabilityMapCalculation()
  {
    ...
    for (int i = 0; i < listSize; ++i)
    {
      ...
      Eigen::MatrixXf matPoints =
          Eigen::Map<const Eigen::Matrix<float, -1, -1, Eigen::RowMajor>>(xyzVector.data(), xyzVector.size() / 3, 3);
  ...
  void publishLocalMap()
  {
    ...
    float originCubeOriginX, originCubeOriginY;  // the orign of submap that the local map origin belongs to (note the
                                                 // submap may not be created yet, cannot use originX and originY)
    ...
    occupancyMap2DHeight.occupancy.info.origin.position.z =
        localMapOriginPoint.z + 10;  // add 10, just for visualization
  ...
  void initializeLocalOccupancyMap()
  {
    ...
    occupancyMap2DHeight.occupancy.data.resize(occupancyMap2DHeight.occupancy.info.width *
                                               occupancyMap2DHeight.occupancy.info.height);
    occupancyMap2DHeight.height.resize(occupancyMap2DHeight.occupancy.info.width *
                                       occupancyMap2DHeight.occupancy.info.height);
    occupancyMap2DHeight.costMap.resize(occupancyMap2DHeight.occupancy.info.width *
                                        occupancyMap2DHeight.occupancy.info.height);
  }
  ...
  void publishTraversabilityMap()
  {
    ...
    for (int i = -visualLength; i <= visualLength; ++i)
    {
      for (int j = -visualLength; j <= visualLength; ++j)
      {
        ...
        if (idx < 0 || idx >= mapArrayLength || idy < 0 || idy >= mapArrayLength)
          continue;
